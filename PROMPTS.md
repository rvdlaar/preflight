...
| A8 | "Produce an SBOM (Software Bill of Materials) for Preflight. List every dependency: FastAPI, Milvus, PostgreSQL, NIM, Unstructured.io, PyMuPDF, LlamaParse, MarkItDown, Azure AI Document Intelligence, NeMo Guardrails, Next.js, shadcn/ui, Voyage-3-Large, BGE-M3, Gemini 2.0. For each: license, version policy, CVE scanning approach, and vendor viability assessment." | Victor, Thomas | Appendix: "SBOM & Supply Chain" |
| A9 | "Assess Preflight's own dependencies against AIVG 2022 + Module ICT. For each external provider (LlamaParse, Voyage AI, Google Gemini, NVIDIA NIM, Unstructured.io): exit-clausule, broncode escrow applicability, data return, 24-month version support, NEN 7510 certification status, verwerkersovereenkomst status, data processing location." | Thomas, Nadia | Appendix: "AIVG Self-Assessment" |
| A10 | "Assign a technology radar position (ADOPT/TRIAL/ASSESS/HOLD) to every component in Preflight's stack: FastAPI, Milvus, PostgreSQL, NIM, Ollama, Unstructured.io, PyMuPDF, LlamaParse, MarkItDown, Voyage-3-Large, BGE-M3, Gemini 2.0, NeMo Guardrails, Next.js, shadcn/ui, Tailwind." | Thomas | Section: "Technology Radar Assessment" |
...
| B8 | "Resolve data residency for all external services. For Gemini 2.0, Voyage AI, LlamaParse, Azure AI Document Intelligence: where does data go? Can they be self-hosted? If cloud, which region? What data crosses the hospital boundary? Document the decision: self-host, EER-only cloud, or exclude." | Aisha, Nadia, FG-DPO | Section: "Data Residency Decisions" |
